...
SERVER_PORT=`sed '/spear.addr/!d;s/.*=//' conf/system.properties | tr -d '\r'`
...
JAVA_JMX_OPTS=" -Dcom.sun.management.jmxremote.port=7802 -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.management.jmxremote.authenticate=false "
...
nohup java $JAVA_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS $JAVA_MEM_OPTS $JAVA_GC_OPTS -classpath $CONF_DIR:$LIB_JARS com.newnoriental.base.MinitorBase 2>&1  >> /dev/null &
PIDS=`ps  --no-heading -C java -f --width 1000 | grep "$DEPLOY_DIR" | awk '{print $2}'`
echo "$PROJECT_NAME started at PORT: $SERVER_PORT PID: $PIDS"
echo "Please check log files..."
echo "File=`date +$STDOUT_FILE`"
echo ""
